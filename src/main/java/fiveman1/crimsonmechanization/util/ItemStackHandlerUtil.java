...
public class ItemStackHandlerUtil {
    ...
    public static void processOutputs(ItemStackHandler outputHandler, List<ItemStack> outputs, List<Integer> chances) {
        int slots = outputHandler.getSlots();
        for (int outputNum = 0; outputNum < outputs.size() && random.nextInt(100) < chances.get(outputNum); outputNum++) {
            ItemStack result = outputs.get(outputNum).copy();
