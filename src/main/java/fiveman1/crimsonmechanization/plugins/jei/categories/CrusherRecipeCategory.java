...
public class CrusherRecipeCategory implements IRecipeCategory<BaseMachineRecipe> {
    ...
    public CrusherRecipeCategory(IGuiHelper guiHelper) {
        ResourceLocation location = new ResourceLocation(CrimsonMechanization.MODID, "textures/gui/" + CrusherBlock.NAME + ".png");

        background = guiHelper.createDrawable(location, 38, 22, 129, 44);
        ...
        icon = guiHelper.createDrawableIngredient(new ItemStack(BlockRegistration.CRUSHER_CRIMSON));
    }

    @Override
    public ResourceLocation getUid() {
        return RecipeTypeRegistration.CRUSHER_RECIPE_ID;
    }
    ...
    public String getTitle() {
        return I18n.format("container." + CrimsonMechanization.MODID + "." + CrusherBlock.NAME);
    }
    ...
    public void draw(BaseMachineRecipe recipe, MatrixStack matrixStack, double mouseX, double mouseY) {
        ...
        List<Integer> chances = recipe.getOutputChances();
        int x = 87;
        for (int chance : chances) {
            if (chance < 100) {
                TextUtil.drawCenteredText(matrixStack, chance + "%", x, 36, ColorUtil.GREY, Minecraft.getInstance().fontRenderer);
            }
            x += 24;
        }
    }
